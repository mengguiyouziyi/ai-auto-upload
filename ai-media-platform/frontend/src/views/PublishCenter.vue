...
// API base URL
const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:9001'
...
const confirmPublish = async (tab) => {
  return new Promise((resolve, reject) => {
    ...
    // 调用后端发布API - 增加超时时间
    http.post('/postVideo', publishData, { timeout: 120000 }) // 2分钟超时
    .then(data => {
      if (data.code === 200) {
        tab.publishStatus = {
          message: '发布成功',
    ...
    .catch(error => {
      ...
      if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
        tab.publishStatus = {
          message: '发布处理中，请稍后查看结果',
          type: 'warning'
        }
        // 即使超时也认为是成功，因为后端可能在继续处理
        resolve()
      } else {
        tab.publishStatus = {
